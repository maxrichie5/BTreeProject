	...
	private void split(BTreeNode parentNode, int childIndex, BTreeNode child) {
		...
		if(!child.isLeaf()) { //if child is not a leaf
			for(int j = 1; j <= degree; j++) {
				...
				for(int j = 0; j < (degree-1); j++) { //half the full node's keys
					newNode.addKey(child.getKey(j+degree-1));
					child.getKeys().remove(j+d-1);
				}

				if(!child.isLeaf()) { //if child is not a leaf
					for(int j = 0; j < degree-1; j++) {
						newNode.addChild(child.getChild(j+degree), j);
						child.getChildren().remove(j+degree);
					}
				}

				parentNode.setNumKeys(degree-1); //set the parent node's keys to t-1
